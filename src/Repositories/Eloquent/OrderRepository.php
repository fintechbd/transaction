    ...
    public function list(array $filters = [])
    {
        ...
        if (!empty($filters['search'])) {
            if (is_numeric($filters['search'])) {
                $query->orWhere($modelTable . '.' . $this->model->getKeyName(), 'like', "%{$filters['search']}%")
                    ->orWhere($modelTable . '.amount', 'like', "%{$filters['search']}%")
                    ->orWhere($modelTable . '.converted_amount', 'like', "%{$filters['search']}%");
            } else {
