...
class OrderRepository extends EloquentRepository implements InterfacesOrderRepository
{
    ...
    public function list(array $filters = [])
    {
        ...
        if (isset($filters['parent_id_is_null']) && $filters['parent_id_is_null']) {
            $query->whereNull('orders.parent_id');
        }
        ...
        if (! empty($filters['user_id_sender_receiver_id'])) {
            $query->where(function ($query) use ($filters) {
                return $query->where('orders.user_id', '=', $filters['user_id_sender_receiver_id'])
                    ->orWhere('orders.sender_receiver_id', '=', $filters['user_id_sender_receiver_id']);
            });
        }

        if (! empty($filters['service_type_slug'])) {
        ...
        if (! empty($filters['transaction_id'])) {
            $query->where(function ($query) use ($filters) {
                return $query->where('orders.order_number', '=', $filters['transaction_id'])
