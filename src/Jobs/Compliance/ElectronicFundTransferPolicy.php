...
class ElectronicFundTransferPolicy implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($orderSumAmount >= $this->highThreshold) {
            ...
            $this->remarks = "{$amountFormatted} electronic fund transferred in last 24 hours has crossed the ".\currency($this->highThreshold, $currency).' threshold limit.';
        } elseif ($orderSumAmount >= $this->moderateThreshold) {
            $this->riskProfile = RiskProfile::Moderate;
            $this->remarks = "{$amountFormatted} electronic fund transferred in last 24 hours has crossed the ".\currency($this->moderateThreshold, $currency).' threshold limit.';
        } else {
            $this->riskProfile = RiskProfile::Low;
            $this->remarks = "{$amountFormatted} electronic fund transferred in last 24 hours is below the ".\currency($this->moderateThreshold, $currency).' threshold limit.';
        }
